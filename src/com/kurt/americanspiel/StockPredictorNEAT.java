 ...
 */
public class StockPredictorNEAT {
	static int numToProcess = 360 * 3;


	static int openCloseSets = 6;
	static int expectedSets = 2;
	...
	/*	for (double[] value : values) {
		...
		NEATPopulation pop = new NEATPopulation(nInput, expectedSets * 2, 10);
		...
		} while (train.getError() > 0.16);

		NEATNetwork network = (NEATNetwork) train.getCODEC().decode(train.getBestGenome());
		/*BasicNetwork network = new BasicNetwork();
		network.addLayer(new BasicLayer(null, true, nInput));
		network.addLayer(new BasicLayer(new ActivationLOG(), true, (nInput * 2) - 1));
		network.addLayer(new BasicLayer(new ActivationLOG(), true, (nInput * 2) - 1));
		network.addLayer(new BasicLayer(new ActivationLOG(), true, (nInput * 2) - 1));
		network.addLayer(new BasicLayer(new ActivationLOG(), true, (nInput * 2) - 1));
		network.addLayer(new BasicLayer(new ActivationLOG(), false, expectedSets * 2));
